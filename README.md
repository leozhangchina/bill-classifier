...
`classify_bill_light.py` 专门适配微信支付导出的 `.xlsx`、支付宝导出的 `.csv` 和招商银行交易流水 `.pdf`：

- 自动定位真实表头；微信样本会删除前 17 行说明，支付宝样本会删除前 23 行说明；
- 自动读取支付宝常见的 GB18030 编码并移除空白尾列；
- 招商银行 PDF 会按固定栏位还原跨行交易记录，并将“客户摘要”最后一个 `-` 后的内容提取为“交易对方”；
- 银行 PDF 会先按原始金额正负判断收入或支出，再将输出的交易金额统一保存为正数；交易金额和联机余额均为可计算数值；
...
python .\classify_bill_light.py "D:\path\招商银行交易流水.pdf"
```
